...
The Message Queue has predefined buffer size, which cannot be expanded in runtime. In case when Message Queue has no space left to receive new Message it will stop to receive any request from the Sender. In this case the Sender will start to accumulate Messages instead of sending them to unavailable Message Queue. Such case can lead to chain-queue, serious system crush and difficulties with the problem determination.
