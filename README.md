...
The Message Queue has predefined buffer size, which cannot be expanded in runtime. In case when Message Queue has no space left to receive new Message it will stop to receive any Messages. In this case the Sender will lost possibility tostop to receive the the requests from previous Sender in the chain and its Message Queue will start to accumulate the message too. Such situation can lead to some serious system crush and difficulties with the problem determination.
